...
type TimerValue={timer:TimerData;setTimer:Dispatch<SetStateAction<TimerData>>;left:number;start:(seconds?:number)=>void};
...
export function TimerProvider({children}:{children:ReactNode}){
  ...
  function start(seconds?:number){if(sound){audio.current??=new AudioContext();void audio.current.resume();}const remaining=seconds??(left||timer.duration);setNow(Date.now());completed.current=null;setTimer({...timer,duration:seconds??timer.duration,remaining,endsAt:Date.now()+remaining*1000,finished:false});}
