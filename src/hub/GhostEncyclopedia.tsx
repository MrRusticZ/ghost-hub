...
import {GhostDossier,GhostEmblem} from './GhostDossier';
...
export function GhostLibrary({id}:{id?:string}){
  ...
  if(ghost){
    ...
    return <div className="ency-page"><Link className="back-link" to="ghosts"><ArrowLeft size={15}/>Ghost encyclopaedia</Link><GhostDossier key={ghost.id} ghost={ghost} onPlay={audio.stop} audioActive={Boolean(audio.active)}/><h2>Field tools & evidence</h2><ActiveFieldTimer/>{matchControls}{soundNote}
  ...
  return <div className="ency-page"><PageHeading eyebrow="EXPLORE / THE ENTITIES" title="Ghost encyclopaedia" description={`${catalog.ghosts.length} ghosts. Find a clue, listen to a pace, compare the timing.`}/>
    ...
    {view==='comparison'?<div className="ency-table-scroll" role="region" aria-label="Ghost details comparison" tabIndex={0}><table className="ency-table"><caption>{comparing?'Selected ghosts':'Ghost comparison'} · base speeds at 100%; player uses your match settings</caption><thead><tr><th scope="col">Ghost / evidence</th><th scope="col">Footsteps</th><th scope="col">Timing</th><th scope="col">Distinguishing test</th></tr></thead><tbody>{displayed.map(g=>{const profile=mechanicsFor(g.id);return <tr key={g.id} data-ghost-entry={g.id}><th scope="row"><Link to={'ghosts/'+g.id} className="ency-name"><GhostEmblem id={g.id}/>{g.name}<ArrowUpRight size={16}/></Link><EvidenceTags evidence={g.evidence}/>{compareCheck(g)}<Link to={'ghosts/'+g.id} className="ency-dossier-link">Open dossier</Link></th><td>{profile?<><FootstepPlayer profile={profile} name={g.name} audio={audio} settings={settings}/><p>{profile.movement}</p></>:<p>Not yet reviewed</p>}</td><td>{profile?<GhostTiming profile={profile} name={g.name} settings={settings}/>:<p>Not yet reviewed</p>}</td><td><p>{g.test}</p><details><summary>What can mislead you</summary><p>{g.caution}</p></details>{profile&&<References profile={profile}/>}</td></tr>;})}</tbody></table></div>:<div className={view==='cards'?'ency-cards':'ency-list'}>{matching.map(g=>{const profile=mechanicsFor(g.id);return <article className="ency-entry" key={g.id} data-ghost-entry={g.id}><div className="ency-entry-heading"><GhostEmblem id={g.id}/><Link to={'ghosts/'+g.id} className="ency-name">{g.name}<ArrowUpRight size={17}/></Link>{compareCheck(g)}</div><EvidenceTags evidence={g.evidence}/>{view==='cards'&&<p className="ency-description">{g.summary}</p>}{profile&&<MechanicsSummary profile={profile}/>}
