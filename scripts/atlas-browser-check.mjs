...
async function ready(p){await p.locator('.atlas-floor-crop img').waitFor();await p.waitForFunction(()=>{const img=document.querySelector('.atlas-floor-crop img');return img?.complete&&img.naturalWidth>0;});await p.evaluate(()=>document.fonts.ready);}
...
try{
  ...
  assert.match(await p.locator('.atlas-floor-crop img').getAttribute('src'),/tanglewood/);
  ...
  let markers=await stored(p);assert.equal(markers.length,1);assert.ok(Math.abs(markers[0].x-60)<1);assert.equal(markers[0].surface,'reference');
  ...
  await select(p,'13 Willow Street');assert.match(await p.locator('.atlas-floor-crop img').getAttribute('src'),/willow/);await expect(p.locator('.atlas-canvas-footer')).toContainText('21 Jul 2026');
  ...
  const failed=await context(),f=await failed.newPage();await f.route('**/*tanglewood*.png*',route=>route.request().resourceType()==='image'?route.abort():route.continue());await f.goto(base+'#/maps');await f.getByRole('heading',{name:'This floor image could not load',exact:true}).waitFor();assert.equal(await f.getByRole('button',{name:'Add at centre',exact:true}).isDisabled(),true);await failed.close();results.push('A failed map image shows an actionable error and cannot accept misleading markers');
